...
class Montagne : public Dessinable //est ce qu'on a deux definitions de montagne quelque part?
{
	...
	void affiche_para();
	void dessine_sur(SupportADessin &a_dessiner) override ; //j'ai retiré le virtual ici
...
class ChaineDeMontagnes : public Montagne
{
	...
	ChaineDeMontagnes(double x, double y, double h, double ox, double oy, const unsigned int &nombre) : Montagne(x, y, h, ox, oy), nombre_montagnes(nombre) //constructeur avec uniquement des montagnes simples
	{
	...
	ChaineDeMontagnes(ChaineDeMontagnes	chaine_a_mettre, const unsigned int &nombre2) : Montagne(0, 0, 0, 0, 0), nombre_chaines(nombre2) //constructeur avec uniquement des chaines
	{
	...
	ChaineDeMontagnes(const unsigned int &nombre, const unsigned int &nombre2) : Montagne(0, 0, 0, 0, 0), nombre_montagnes(nombre), nombre_chaines(nombre2) //constructeur avec les deux types
	{
		Montagne mon;
		for (size_t i(0); i < nombre_montagnes; ++i)
		{
	...
	int get_nbre_montagnes(); //getters
	int get_nbre_chaines();
	
	void set_montagne(double x, double y, double h, double ox, double oy, const unsigned int &indice); //setters
